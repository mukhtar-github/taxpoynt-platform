...
DEFAULT_NAMESPACE = 'urn:oasis:names:specification:ubl:schema:xsd:Invoice-2'
...
class OdooUBLTransformer:
    ...
    def ubl_object_to_xml(self, ubl_invoice: InvoiceValidationRequest) -> Tuple[str, List[Dict[str, str]]]:
        ...
        try:
            # Create XML root element with default namespace
            root = ET.Element(f"{{{DEFAULT_NAMESPACE}}}Invoice")
            
            # The namespaces are already registered in __init__, so they will be added automatically
            # when ET.tostring() is called
    ...
    def _add_element(self, parent: ET.Element, tag: str, text: Any) -> ET.Element:
        ...
        # Convert tag to full namespace if it has a prefix
        if ':' in tag:
            prefix, local_name = tag.split(':', 1)
            if prefix in NAMESPACES:
                tag = f"{{{NAMESPACES[prefix]}}}{local_name}"
