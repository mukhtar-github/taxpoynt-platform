...
class ValidationEndpointsV1:
    ...
    **Available Validation Services:**
    ...
    async def get_available_validation_services(self, request: Request):
    ...
    async def get_bvn_validation_status(self, validation_id: str, request: Request, context: HTTPRoutingContext = Depends(lambda: None)):
    ...
    async def get_kyc_status(self, kyc_id: str, request: Request, context: HTTPRoutingContext = Depends(lambda: None)):
        """Get KYC status"""
        try:
            context = await self._require_si_role(request)
            result = await self.message_router.route_message(
    ...
    async def get_kyc_details(self, kyc_id: str, request: Request, context: HTTPRoutingContext = Depends(lambda: None)):
        """Get KYC details"""
        try:
            context = await self._require_si_role(request)
            result = await self.message_router.route_message(
    ...
    async def get_identity_verification_status(self, verification_id: str, request: Request, context: HTTPRoutingContext = Depends(lambda: None)):
    ...
    async def get_validation_service_health(self, service_id: str, request: Request, context: HTTPRoutingContext = Depends(lambda: None)):
