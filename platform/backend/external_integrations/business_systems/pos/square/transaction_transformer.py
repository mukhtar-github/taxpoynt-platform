"""
Square POS Transaction Transformer

Transforms Square POS transactions to FIRS-compliant UBL BIS 3.0 invoices
...
class SquareTransactionTransformer:
    """
    Square POS transaction to FIRS invoice transformer - System Integrator Functions.
    
    Transforms Square transactions into UBL BIS 3.0 compliant invoices
    ...
    def __init__(self, config: Dict[str, Any]):
        """Initialize Square transaction transformer."""
        self.config = config
        ...
        self.invoice_prefix = self.invoice_config.get('prefix', 'SQ')
    ...
    async def transform_transaction_to_invoice(
        ...
        """
        Transform Square transaction to FIRS-compliant UBL invoice.
        ...
        try:
            ...
            self.logger.info(f"Transforming Square transaction {transaction.transaction_id} to FIRS invoice")
            
            # Generate invoice header
            invoice_header = self._build_invoice_header(transaction, options)
            
            # Build supplier party (merchant)
    ...
    def _build_invoice_header(self, transaction: POSTransaction, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'order_reference': transaction.metadata.get('square_payment_id'),
    ...
    def _build_supplier_party(self, transaction: POSTransaction, options: Dict[str, Any]) -> Dict[str, Any]:
        """Build UBL supplier party (merchant)."""
    ...
    def _build_customer_party(self, transaction: POSTransaction, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        address = {
            'street_name': customer_address.get('address_line_1', 'N/A'),
            'additional_street_name': customer_address.get('address_line_2', ''),
            'city_name': customer_address.get('city', 'Lagos'),
            'postal_zone': customer_address.get('postal_code', ''),
            'country_subdivision': customer_address.get('state', 'Lagos'),
            'country': {
                'identification_code': customer_address.get('country', 'NG'),
                'name': 'Nigeria'
            }
        }
    ...
    def _transform_item_to_invoice_line(self, item: Dict[str, Any], line_number: int, transaction: POSTransaction) -> Dict[str, Any]:
        ...
        invoice_line = {
            'id': str(line_number),
            'note': f"Square POS item: {item.get('name', 'Unknown Item')}",
            'invoiced_quantity': {
                'value': quantity,
    ...
    def _build_item_properties(self, item: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for modifier in modifiers:
            properties.append({
                'name': 'Modifier',
                'value': f"{modifier.get('name', '')} (₦{modifier.get('total_price', 0)})"
            })
        
        # Add Square-specific metadata
        if item.get('metadata', {}).get('square_item_id'):
            properties.append({
                'name': 'Square Item ID',
                'value': item['metadata']['square_item_id']
            })
    ...
    def _build_payment_terms(self, transaction: POSTransaction, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'payment_means': [{
                ...
                'payment_means_explanation': f"Square POS - {transaction.payment_method}",
                'payment_due_date': transaction.timestamp.date().isoformat(),
                'instruction_id': transaction.transaction_id,
                'instruction_note': payment_terms.get('note', 'Payment processed via Square POS system')
            }],
            'payment_terms': {
    ...
    def _map_payment_method_code(self, payment_method: str) -> str:
        ...
        mapping = {
            ...
            'MOBILE_MONEY': '68',  # Online payment service
    ...
    def _build_document_references(self, transaction: POSTransaction) -> List[Dict[str, Any]]:
        ...
        # Add Square receipt reference if available
        receipt_url = transaction.metadata.get('receipt_url')
        if receipt_url:
            references.append({
                'id': transaction.metadata.get('receipt_number', transaction.transaction_id),
                'document_type_code': 'receipt',
                'document_description': 'Square POS Receipt',
                'attachment': {
                    'external_reference': {
                        'uri': receipt_url
                    }
        ...
        order_id = transaction.metadata.get('square_payment_id')
        if order_id:
            references.append({
                'id': order_id,
                'document_type_code': 'order',
                'document_description': 'Square Order Reference'
            })
    ...
    def _build_invoice_metadata(self, transaction: POSTransaction, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'square_location_id': transaction.location_id,
            ...
            'processing_fee': transaction.metadata.get('processing_fee'),
            'card_details': transaction.metadata.get('card_details'),
            'risk_evaluation': transaction.metadata.get('risk_evaluation')
        }
