#!/usr/bin/env python3
"""
TaxPoynt eInvoice - FIRS API Endpoint Tester (v1)

Tests basic FIRS API endpoints to determine which ones are accessible
with the current credentials and configuration.
"""
import argparse
import datetime
import json
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FIRS API Endpoint Tester (v1)")
    parser.add_argument("--invoice", help="Invoice number to use for tests", default="INV001")
    args = parser.parse_args()
    
    # Print welcome banner
    print("="*70)
    print("TaxPoynt eInvoice - FIRS API Endpoint Tester (v1)")
    print("="*70)
    ...
    # 1. Test currencies endpoint (should work without authentication)
    results["currencies"] = test_resources_endpoint()
    
    # 2. Test VAT exemptions endpoint (should work without authentication)
    results["vat_exemptions"] = test_vat_exemptions_endpoint()
    
    # 3. Test IRN validation endpoint
    results["irn_validation"] = test_irn_validation_endpoint(args.invoice)
    ...
    print("\nTest complete. Use these results to determine which endpoints to include in your demo.")
    print("="*70)
