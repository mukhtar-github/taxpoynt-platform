"""
FIRS SI IRN Generation Service

This service implements System Integrator (SI) specific functionality for
Invoice Reference Number (IRN) generation and QR code creation.

SI Role Responsibilities:
- Generate unique IRNs based on invoice data
- Create QR codes for invoice verification
- Validate IRNs against FIRS requirements
- Manage IRN lifecycle (creation, validation, expiration)
- Bulk IRN generation for batch processing
- IRN caching for performance optimization
- Audit logging for compliance and traceability

This service is part of the firs_si package and focuses on the SI's core
responsibility of IRN generation and QR code creation.
"""
import uuid
import hashlib
import hmac
